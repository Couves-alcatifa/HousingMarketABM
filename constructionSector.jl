...
function initiateConstructionSector()
    ...
    housesInConstruction = Dict(location => Dict(size_interval => PendingConstruction[] for size_interval in instances(SizeInterval)) for location in [Lisboa])
    ...
    return ConstructionSector(STARTING_CONSTRUCTION_SECTOR_WEALTH, 
            ...
            Mortgage[],
            Dict(location => 0.0 for location in [Lisboa]),
            PendingRenovation[]
            )
end
